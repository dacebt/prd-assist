...
- `Home` → `onResize(valueNow - valueMin)`, `End` → `onResize(valueNow - valueMax)`. (Sign matches the subtractive caller convention `setWidth(width - dx)` established in §"SessionPage structure" line 154. See Adaptation Log 2026-04-20.)
...
- **2026-04-20 — Visibility refetch must not flash the loading state (rival checkpoint, slice 10).** Initial `SessionList.load()` unconditionally set `status: "loading"` before every fetch, which made the visibility refetch blank the rendered list on every tab-return — a UX regression the spec never sanctioned (Scenario 12 says "shows the new session," not "shows a loading flash"). Resolution: `load(mode)` parameter — `"initial"` (mount and retry) clears to loading; `"refresh"` (visibilitychange) leaves the current `loaded` state in place and updates on success. Error path still sets `status: "error"` from either mode.
- **2026-04-20 — Slice order: 7 before 6.** Slice 6's empty-state inline "New session" button must share `useCreateSession` with the header button. Doing slice 7 first (extract hook + migrate header `NewSessionButton`) lets slice 6 use the hook for both call sites without duplicating logic. No scope change.
- **2026-04-20 — Home/End sign fix (coherence concern, slice 4).** Spec internally inconsistent: line 154 prescribes `setWidth(width - dx)` (subtractive) while line 224 prescribed Home/End deltas in additive form (`valueMin - valueNow`, `valueMax - valueNow`). With both rules applied, Home and End snapped to opposite extremes (Home expanded, End clamped to min). Resolution: invert Home/End in `ResizeHandle.keyboardDelta` to match the subtractive convention (`valueNow - valueMin`, `valueNow - valueMax`). Arrow ±16/±64 unaffected (symmetric under negation). Fix is localized to `ResizeHandle.tsx`; pointer path and SessionPage onResize wire unchanged.
- **2026-04-20 — Sequencing: slice 8 folded into slice 4.** The `PrdPane` `onClose` removal and `ChatPane` `border-r`/`autoFocus` changes are coupled to the `SessionPage` rewrite — keeping them in a separate slice 8 forces either (a) an ugly intermediate state with two PRD close controls, or (b) pre-emptive surgery on the old SessionPage. Folding into slice 4 leaves every commit clean. Slice 8 is now a no-op; slice 10 verification unchanged.
